<template>
  <div class="space-y-6">
    <div class="flex items-center gap-4">
      ...
      <div class="flex-1">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">行業別代碼</h1>
        ...
        <table class="data-table">
          ...
          <tbody>
            ...
            <tr v-else-if="fetchError">
              <td colspan="3" class="text-center py-12 text-eco-red-500 text-sm">載入失敗：{{ fetchError }}</td>
            </tr>
      ...
      <div v-if="modal.show" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-white dark:bg-dark-800 rounded-xl shadow-xl w-full max-w-sm p-6">
          <h3 class="font-semibold text-gray-900 dark:text-white mb-4">
            {{ modal.form.id ? '編輯行業代碼' : '新增行業代碼' }}
          </h3>
          <div class="space-y-4">
            <div>
              <label class="form-label">行業代號 <span class="text-eco-red-500">*</span></label>
              <input
                v-model="modal.form.industry_code"
                type="text"
                class="form-input"
                placeholder="例：01"
            ...
            <div>
              <label class="form-label">行業名稱</label>
              <input v-model="modal.form.industry_name" type="text" class="form-input" placeholder="行業名稱">
            </div>
          </div>
...
const save = async () => {
  if (!modal.form.industry_code) {
    errorMsg.value = '行業代號為必填'
    return
  }
  errorMsg.value = ''
  modal.saving = true
  try {
    await $fetch('/api/psi/industry-codes', {
      method: 'POST',
      body: modal.form,
